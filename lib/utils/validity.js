...
const checkValidPost = (fixtureName, server, _fixture, _options) => {
  ...
  const before = options.before || Promise.resolve;
...
const checkValidPatch = (fixtureName, server, _fixture, _options) => {
  ...
  let compareFixture = fixture;
  if (_options.compareWith) {
    compareFixture = _options.compareWith;
  }
  return seed
    ...
    .then((res) => checkBody(res, compareFixture, fixtureName))
    .then(() => db.getModel(server, fixtureName, id))
    .then(dbResult => compareWithFixture(dbResult, compareFixture, fixtureName));
};
